...
export function AdminDashboard() {
 ...
 <Loader2 className="animate-spin text-sky-600" size={64} />
 </div>
 </DashboardLayout>
 );
 }
 ...
 <button className="px-6 py-3 bg-sky-600 text-white rounded-xl text-xs font-bold hover:bg-sky-700 transition-all flex items-center gap-2 shadow-md shadow-sky-100">
 ...
 >
 {tab}
 {activeTab === tab && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-sky-600" />}
 </button>
 ))}
 </div>
 ...
 <Activity size={14} className="text-[#38BDF8]" /> RECENT ACTIVITY
 </div>
 ...
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Budget</th>
 ...
 <p className="text-xs text-slate-400 font-medium mt-0.5">{proj.client?.firstName}</p>
 </td>
 <td className="p-4 text-xs font-bold text-slate-500 uppercase">Project</td>
 ...
 <span className="text-xs font-bold text-slate-500 uppercase">{proj.status}</span>
 </div>
 </td>
 </tr>
 ))}
 ...
 className={`w-12 h-6 rounded-full transition-all relative p-1 ${settings.maintenanceMode ? 'bg-sky-600' : 'bg-slate-200'}`}
 >
 ...
 disabled={isUpdatingSettings}
 className={`w-12 h-6 rounded-full transition-all relative p-1 ${settings.coolingPeriodActive !== false ? 'bg-sky-600' : 'bg-slate-200'}`}
 >
 ...
 <input className="pl-12 pr-6 py-3 bg-white border border-slate-200 rounded-xl w-80 outline-none focus:border-[#38BDF8] transition-all text-sm font-medium" placeholder="Search users..." />
 </div>
 </div>
 <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
 <table className="w-full text-left">
 ...
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">AI Score</th>
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Status</th>
 ...
 {u.isPro && (
 <span className="flex items-center gap-1 px-1.5 py-0.5 bg-gradient-to-r from-amber-400 to-amber-600 text-white text-xs font-bold rounded-full shadow-sm">
 ...
 <td className="p-4"><div className="px-3 py-1 bg-slate-100 rounded-lg text-xs font-bold text-slate-500 uppercase">{u.role}</div></td>
 <td className="p-4">
 <div className="flex items-center gap-3">
 <div className="w-20 bg-slate-100 h-1.5 rounded-full overflow-hidden">
 <div className="bg-sky-600 h-full" style={{ width: `${u.aiScore || 0}%` }} />
 </div>
 <span className="text-xs font-bold text-slate-600">{u.aiScore || 0}%</span>
 </div>
 </td>
 <td className="p-4">
 <div className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${u.isVerified ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-50 text-slate-400'}`}>
 {u.isVerified ? 'Verified' : 'Pending'}
 </div>
 </td>
 <td className="p-4">
 <div className="flex gap-4">
 <button onClick={() => navigate(`/profile/${u._id || u.id}`)} className="text-xs font-bold text-sky-600 hover:text-sky-700 transition-colors">Profile</button>
 ...
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Module</th>
 ...
 <p className="text-xs text-slate-400 font-medium">{audit.user?.email}</p>
 </td>
 <td className="p-4">
 <p className="text-xs font-bold text-slate-900">{audit.assessment?.title || 'General Test'}</p>
 <p className="text-xs text-slate-400 font-medium mt-0.5">{audit.assessment?.category}</p>
 </td>
 <td className="p-4">
 <div className="flex items-center gap-2">
 ...
 <td className="p-4">
 <div className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider ${audit.status === 'passed' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'}`}>
 ...
 <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Total Volume</p>
 <p className="text-3xl font-bold text-sky-600">₹{fullTransactions.reduce((acc, curr) => acc + (curr.amount || 0), 0).toLocaleString()}</p>
 </div>
 </div>
 <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
 <table className="w-full text-left">
 ...
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Recipient</th>
 <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Status</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-slate-100">
 ...
 <td className="p-4">
 <p className="font-mono text-xs font-bold text-sky-600 bg-sky-50 px-2 py-1 rounded-lg w-fit uppercase tracking-wider">
 TX_{txn._id?.slice(-8).toUpperCase() || 'EXT'}
 </p>
 <p className="text-xs text-slate-400 font-medium mt-1.5">{new Date(txn.date).toLocaleDateString()} at {new Date(txn.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
 </td>
 ...
 <p className="text-xs text-slate-400 font-medium mt-0.5">{txn.client?.email}</p>
 </td>
 <td className="p-4">
 <p className="text-sm font-semibold text-slate-900">{txn.professional?.firstName || 'System'} {txn.professional?.lastName || 'Payment'}</p>
 <p className="text-xs text-slate-400 font-medium mt-0.5">{txn.professional?.email || 'internal'}</p>
 </td>
 <td className="p-4">
 <div className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-bold uppercase tracking-wider">Successful</div>
 </td>
 </tr>
 ))}
 </tbody>
...
export function CreateProctoringTest() {
 ...
 <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-slate-400 hover:text-sky-600 mb-6 transition-all group font-bold text-xs">
 ...
 <Zap size={20} className="text-sky-600" /> Test Configuration
 </h3>
 ...
 className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#38BDF8] outline-none transition-all font-medium text-sm"
 ...
 className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#38BDF8] outline-none transition-all font-bold text-sm appearance-none cursor-pointer"
 >
 ...
 className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#38BDF8] outline-none transition-all font-medium text-sm resize-none"
 ...
 className="px-4 py-2 bg-sky-600 text-white rounded-xl text-xs font-bold hover:bg-sky-700 transition-all flex items-center gap-2"
 >
 ...
 <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Question {qIdx + 1}</span>
 <button 
 type="button"
 ...
 <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Option {String.fromCharCode(65 + oIdx)}</label>
 <input 
 type="radio" 
 name={`correct-${qIdx}`} 
 ...
 type="submit" 
 disabled={isLoading}
 className="px-12 py-3 bg-sky-600 text-white font-bold rounded-xl text-sm hover:bg-sky-700 transition-all flex items-center gap-3 shadow-md shadow-sky-100"
 >
...
function StatCard({ label, value, icon: Icon, color }) {
 ...
 return (
 <div className="p-6 bg-white border border-slate-200 rounded-2xl shadow-sm hover:border-sky-300 transition-all group">
