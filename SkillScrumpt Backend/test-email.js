...
const runMailTest = async () => {
  const targetEmail = 'marothihemasundar03@gmail.com';

  console.log('--------------------------------------------------');
  console.log(`STARTING COMPREHENSIVE MAIL TEST FOR: ${targetEmail}`);
  console.log('--------------------------------------------------');

  const testData = [
    { name: 'Welcome Student', template: templates.welcomeStudent('John Doe') },
    { name: 'Welcome Client', template: templates.welcomeClient('Acme Corp') },
    { name: 'Assessment Pass', template: templates.assessmentResult('John Doe', 'Full Stack Development', 92, 'passed', 'Certified Expert') },
    { name: 'Assessment Fail', template: templates.assessmentResult('John Doe', 'Data Science', 45, 'failed', null) },
    { name: 'Password Reset', template: templates.forgotPassword('John Doe', 'https://skillscrumpt.vercel.app/reset/token123') },
    { name: 'Desktop Handoff', template: templates.desktopHandoff('https://skillscrumpt.vercel.app/test/start/456') },
    { name: 'Pro Upgrade', template: templates.proUpgrade('John Doe') }
  ];

  for (const test of testData) {
    try {
      console.log(`[ ] Sending: ${test.name}...`);
      await sendEmail(targetEmail, `[TEST] ${test.template.subject}`, test.template.html);
      console.log(`[✓] Successfully sent: ${test.name}`);
    } catch (error) {
      console.error(`[X] Failed to send: ${test.name}`);
      console.error(`    Error: ${error.message}`);
    }
    // Small delay to avoid Gmail rate limiting during test
    await new Promise(resolve => setTimeout(resolve, 1000));
  }

  console.log('--------------------------------------------------');
  console.log('MAIL TEST SEQUENCE COMPLETE');
  console.log('--------------------------------------------------');
};
